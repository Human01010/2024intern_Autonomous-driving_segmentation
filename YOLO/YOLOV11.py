...
def main():
    ...
    results = model.train(
        ...
        name='yolov11-seg-cityscapes'  # Experiment name
    )
    ...
    best_model = YOLO('runs/segment/yolov11-seg-cityscapes/weights/best.pt')  # Load best weights

    results_csv = os.path.join('runs/segment/yolov11-seg-cityscapes', 'results.csv')
    if os.path.exists(results_csv):
        ...
        if 'train/seg_loss' in df.columns:
            ...
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.title('YOLOv11 Training Loss Curve')
            plt.grid()
            plt.show()
        else:
    ...
    if not sample_image_paths:
        ...
        for i, img_path in enumerate(sample_image_paths):
            ...
            axs[i].axis('off')
        plt.suptitle('YOLOv11 Segmentation Results')
        plt.show()
